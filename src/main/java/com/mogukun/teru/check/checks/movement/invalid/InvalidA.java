package com.mogukun.teru.check.checks.movement.invalid;
...
public class InvalidA extends Check {
    ...
    public void onMove(TeruTickEvent e){
        ...
        boolean isMathGround = e.getTo().getY() % (1/64D) < 1E-4; // Credit: DerRedstoner's CheatGuard.


        int flagged = 0;

        if ( player.isOnGround() ){
            if ( ( e.getDeltaY() != 0 && e.getLastDeltaY() != 0 ) || ( !isMathGround && ( e.getDeltaY() != 0 || e.getLastDeltaY() != 0 ) ) ){
                flagged = 1;
            }
        }else {
        ...
        if ( flagged == 3 ){
            ...
            violation_c.get(uuid).count();
            int count = violation_c.get(uuid).getCount();
